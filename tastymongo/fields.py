...
class ToManyField( RelatedField ):
    ...
    def hydrate( self, bundle ):
        ...
        try:
            # FIXME: value should exist but we find the API posting 'null' sometimes. 
            # Explore the options but ignore for now hence the check `if value`
            return [self.build_related_bundle( value, request=bundle.request ) for value in values_list if value]
        except TypeError:
